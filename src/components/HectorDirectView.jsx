...
const HECTOR_RUTA_RULES = [
  ...
  "3.b. Excepción: incluso si no necesitas 'datos vivos' del trayecto, gasta un web_search para confirmar las direcciones de las paradas concretas que vayas a emitir (áreas de servicio, ventas, gasolineras). Esto puebla el campo direccion y le permite al CEO guardar la parada con su dirección real.",
  ...
  '    {"tipo":"cafe","lugar":"Área Servicio Antequera","direccion":"A-92 km 145, 29200 Antequera","hora":"09:15","km":120,"nota":"15 min · gasolina si toca"},',
  '    {"tipo":"comida","lugar":"Venta El Romeral, Córdoba","direccion":"N-IV km 320, 14710 Córdoba","hora":"11:30","km":280,"nota":"Te gustó con Marc en mayo · andaluza · 1h","fromCeoPlace":true},',
  '    {"tipo":"gasolina","lugar":"Cepsa Manzanares","direccion":"A-4 km 154, 13200 Manzanares","hora":"13:00","km":480,"nota":""},',
  '    {"tipo":"destino","lugar":"Madrid","hora":"13:45","km":590,"nota":""}',
  '  ]',
  '}',
  "",
  "Campos obligatorios: paradas (array, mínimo 2 items con lugar string no vacío). Todo lo demás (etaTotal, distanciaTotal, peajesEstimados, salida, hora/km/nota/direccion de cada parada, fromCeoPlace) es OPCIONAL — emítelo solo si aplica. NO inventes cifras si no las buscaste. Sin invención.",
  "",
  ...
  "",
  "Campo direccion en cada parada — por defecto SÍ debes emitirlo para toda parada física concreta (cafe, comida, gasolina, descanso, destino con nombre propio). Para localizarlo: usa uno de tus 2 web_search del turno para confirmar direcciones cuando la ruta lleve paradas nuevas — merece la pena gastarlo aquí. La dirección puede ser postal clásica ('C. Tundidores 3, 14002 Córdoba') O identificación de carretera con punto kilométrico y salida ('A-92 km 145, salida Antequera Norte, 29200 Antequera'). Ambas cuentan como 'direccion clara'. Solo OMITE el campo si tras buscar no encontraste NI dirección postal NI punto kilométrico + salida — en ese caso deja el campo fuera. NUNCA inventes, aproximes ni reconstruyas — el CEO prefiere vacío a erróneo. No la pongas en paradas tipo inicio o destino que son ciudades genéricas ('Marbella', 'Madrid') donde una dirección no aporta.",
  "",
