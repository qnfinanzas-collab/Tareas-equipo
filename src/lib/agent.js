...
export const SKILL_TRIGGERS = {
  ...
  gobernanza:  ["holding", "filial", "estructura societaria", "consolidación fiscal", "consolidacion fiscal", "participation exemption", "transfer pricing", "precios transferencia", "sucesión", "sucesion", "patrimonio", "patrimonial", "gobernanza", "gobierno corporativo", "compliance", "reestructuración", "reestructuracion", "fusión", "fusion", "escisión", "escision", "canje valores", "empresa familiar", "protocolo familiar", "doble imposición", "doble imposicion", "cdi", "establecimiento permanente", "cfc rules", "tax rate", "optimización fiscal", "optimizacion fiscal", "reserva capitalización", "reserva capitalizacion", "junta socios", "junta general", "cuentas anuales", "registro mercantil", "depósito cuentas", "deposito cuentas", "modelo 200", "modelo 202", "impuesto sociedades", "administrador", "acta junta"],
};
...
export function loadSkillContext(skill){
  const skillSummaries = {
    ...
    gobernanza: `MODO GOBERNANZA EMPRESARIAL: Diseña estructuras societarias óptimas (SL, holding, patrimonial, SPV). Participation exemption 95% en dividendos filial→holding. Consolidación fiscal si ≥75% participación. Conoce internacionalización (CDIs, CFC rules, sustancia real, jurisdicciones UE y global). Calendario fiscal completo mes a mes (modelos 111, 200, 202, 303, 347, 390, 720). Obligaciones societarias (junta general 6 meses, depósito cuentas 1 mes post-junta, legalización libros 4 meses, auditoría). Planificación sucesoria con bonificación 95% ISD empresa familiar. Transfer pricing, reservas capitalización/nivelación. Aplica LSC, LIS 27/2014, LAU, ATAD, DAC6, RGPD.`,
  };
...
export function getEnergyLevel(hour){
  if(hour>=6 && hour<9)  return "muy alta";
  if(hour>=9 && hour<12) return "alta";
  if(hour>=12&& hour<14) return "media (post-comida)";
  if(hour>=14&& hour<17) return "media-alta";
  if(hour>=17&& hour<19) return "media-baja";
  return "baja";
}
